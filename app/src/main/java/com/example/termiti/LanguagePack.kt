    ...
    companion object {
        ...
        private fun buildStrings(code: String, s: JSONObject, fb: AppStrings?): AppStrings {
            ...
            return AppStrings(
                ...
                castleOutlawCamp         = str("castleOutlawCamp",         "Tábor psanců"),
                castleVariant            = str("castleVariant",            "Hrad %d"),
                wallClassic              = str("wallClassic",              "Klasická"),
                wallVariant              = str("wallVariant",              "Hradba %d"),
                ...
                backShort = str("backShort", "← Zpět"),
                opponentDefault = str("opponentDefault", "Soupeř"),
                mpQuickMatch = str("mpQuickMatch", "RYCHLÝ ZÁPAS"),
                mpSuperRandom = str("mpSuperRandom", "SUPER NÁHODNÝ"),
                mpLeaderboard = str("mpLeaderboard", "ŽEBŘÍČEK"),
                mpDisconnect = str("mpDisconnect", "ODPOJIT"),
                mpModeSuperRandom = str("mpModeSuperRandom", "Super Náhodný"),
                mpModeQuick = str("mpModeQuick", "Rychlý zápas"),
                mpDeckLabel = str("mpDeckLabel", "BALÍČEK"),
                mpQueue = str("mpQueue", "Fronta"),
                mpGamesSuffix = str("mpGamesSuffix", "(%d her)"),
                mpConnectHint = str("mpConnectHint", "Připoj se k lobby serveru a najdi soupeře"),
                mpNickname = str("mpNickname", "Přezdívka"),
                mpConnect = str("mpConnect", "PŘIPOJIT SE"),
                mpConnecting = str("mpConnecting", "Připojuji k serveru…"),
                mpCancel = str("mpCancel", "← ZRUŠIT"),
                mpSearching = str("mpSearching", "Hledám soupeře…"),
                mpInQueue = str("mpInQueue", "Ve frontě: %d hráčů"),
                mpBackPlain = str("mpBackPlain", "ZPĚT"),
                mpOpponentFound = str("mpOpponentFound", "SOUPEŘ NALEZEN!"),
                mpPreparing = str("mpPreparing", "Připravuji hru…"),
                mpConnectionError = str("mpConnectionError", "CHYBA PŘIPOJENÍ"),
                mpRetry = str("mpRetry", "ZKUSIT ZNOVU"),
                errUnknown = str("errUnknown", "neznámá chyba"),
                errNetAbort = str("errNetAbort", "spojení přerušeno sítí"),
                errRefused = str("errRefused", "server nepřijímá spojení"),
                errNoHost = str("errNoHost", "nelze najít server"),
                errNotFound = str("errNotFound", "server nenalezen"),
                errTimeout = str("errTimeout", "vypršel čas připojení"),
                errReset = str("errReset", "spojení resetováno"),
                errUnreachable = str("errUnreachable", "síť nedostupná"),
                errRejected = str("errRejected", "server odmítl spojení"),
                errUnknownConn = str("errUnknownConn", "neznámá chyba připojení"),
                lobbyDeckReady = str("lobbyDeckReady", "Balíček: %d karet připraveno"),
                lobbyDeckRandom = str("lobbyDeckRandom", "Balíček: náhodný"),
                lobbyEnterNick = str("lobbyEnterNick", "Zadej přezdívku"),
                lobbySearchingSuperRandom = str("lobbySearchingSuperRandom", "Hledám super náhodného soupeře…"),
                lobbyDeckNot30 = str("lobbyDeckNot30", "Vybraný balíček nemá 30 karet"),
                lobbyConnectFailed = str("lobbyConnectFailed", "Nepodařilo se připojit: %s"),
                lobbyConnectionLost = str("lobbyConnectionLost", "Spojení přerušeno (kód %s)"),
                lobbyConnected = str("lobbyConnected", "Připojeno ✓"),
                lobbyInQueue = str("lobbyInQueue", "Ve frontě…"),
                lobbyOpponentFoundPreparing = str("lobbyOpponentFoundPreparing", "Soupeř nalezen! Připravuji hru…"),
                lobbyOpponentLeftWin = str("lobbyOpponentLeftWin", "Soupeř se odpojil – vyhráváš!"),
                lobbyOutdated = str("lobbyOutdated", "Tvá verze hry je zastaralá. Aktualizuj aplikaci pro hraní online."),
                shopPacks = str("shopPacks", "BALÍČKY"),
                shopPackInfo = str("shopPackInfo", "5 karet • 1× vzácná nebo lepší garantována"),
                shopBuyPack = str("shopBuyPack", "KOUPIT BALÍČEK"),
                shopCanBuy = str("shopCanBuy", "Můžeš koupit %d× balíček"),
                shopEarnGold = str("shopEarnGold", "Zlato získáš vítězstvím v bitvě"),
                shopPackOpened = str("shopPackOpened", "BALÍČEK OTEVŘEN!"),
                shopTapToReveal = str("shopTapToReveal", "Klepni na kartu pro odkrytí"),
                shopDuplicates = str("shopDuplicates", "Duplikáty → +%d prachu"),
                shopFinish = str("shopFinish", "DOKONČIT"),
                lbLoadFailed = str("lbLoadFailed", "Nepodařilo se načíst žebříček\n%s"),
                lbTitle = str("lbTitle", "🏆 ŽEBŘÍČEK"),
                lbTotalPlayers = str("lbTotalPlayers", "%d hráčů celkem"),
                lbModeSuperRandom = str("lbModeSuperRandom", "🌪️ Super Náhodný"),
                lbLoading = str("lbLoading", "Načítám žebříček…"),
                lbRetry = str("lbRetry", "🔄 Zkusit znovu"),
                lbEmpty = str("lbEmpty", "Žebříček je prázdný"),
                lbPlayer = str("lbPlayer", "Hráč"),
                stay = str("stay", "Zůstat"),
                leave = str("leave", "Odejít"),
                surrender = str("surrender", "Vzdát se"),
                surrenderQ = str("surrenderQ", "Vzdát se?"),
                close = str("close", "Zavřít"),
                closeX = str("closeX", "✕ Zavřít"),
                tapToClose = str("tapToClose", "Klepnutím zavřeš"),
                backToMenuCaps = str("backToMenuCaps", "ZPĚT DO MENU"),
                roundN = str("roundN", "Kolo %d"),
                opponentAcc = str("opponentAcc", "soupeře"),
                statWinShort = str("statWinShort", "V"),
                statLossShort = str("statLossShort", "P"),
                mpStats = str("mpStats", "STATISTIKY"),
                leaveGameQ = str("leaveGameQ", "Opustit hru?"),
                leaveGameMsg = str("leaveGameMsg", "Rozehraná partie bude ztracena. Opravdu chceš odejít do menu?"),
                onlineSurrenderMsg = str("onlineSurrenderMsg", "Vzdát se? Prohra bude zaznamenána a soupeř bude prohlášen vítězem."),
                onlineOppLeft = str("onlineOppLeft", "Soupeř se odpojil"),
                onlineWaitReconnect = str("onlineWaitReconnect", "Čekám na reconnect…"),
                onlineConnLost = str("onlineConnLost", "Ztraceno připojení"),
                onlineReconnecting = str("onlineReconnecting", "Připojuji se zpět…"),
                onlineYouFirst = str("onlineYouFirst", "⚔ Ty začínáš první"),
                onlineOppFirst = str("onlineOppFirst", "⏳ Soupeř začíná první"),
                onlineGameOver = str("onlineGameOver", "Konec hry"),
                onlineDraw = str("onlineDraw", "Remíza!"),
                onlineDrawEqual = str("onlineDrawEqual", "Obě strany mají stejný hrad"),
                onlineVictory = str("onlineVictory", "Vítězství!"),
                onlineDefeat = str("onlineDefeat", "Prohra"),
                onlineYouBeat = str("onlineYouBeat", "Porazil jsi %s"),
                onlineWinnerWon = str("onlineWinnerWon", "%s zvítězil"),
                onlineBackToLobby = str("onlineBackToLobby", "Zpět do lobby"),
                lostCardsTitle = str("lostCardsTitle", "SPÁLENÉ & UKRADENÉ KARTY"),
                lostCardsSubtitle = str("lostCardsSubtitle", "Karty, o které tě připravil soupeř."),
                lostCardsEmpty = str("lostCardsEmpty", "Žádná karta zatím nebyla spálena ani ukradena."),
                lostCardsButton = str("lostCardsButton", "Spálené & ukradené (%d)"),
                badgeStolen = str("badgeStolen", "UKRADENO"),
                badgeBurned = str("badgeBurned", "SPÁLENO"),
                inspectGameCaps = str("inspectGameCaps", "PROHLÉDNOUT HRU"),
                handCount = str("handCount", "RUKA (%d)"),
                actionWait = str("actionWait", "Čekat"),
                logGameStarts = str("logGameStarts", "— Hra začíná —"),
                arenaTitleDraft = str("arenaTitleDraft", "ARÉNA — DRAFT"),
                arenaPickOne = str("arenaPickOne", "Vyber jednu kartu"),
                arenaPick = str("arenaPick", "VYBRAT"),
                arenaDeckComposition = str("arenaDeckComposition", "SLOŽENÍ BALÍČKU"),
                arenaEffects = str("arenaEffects", "Efekty"),
                arenaRecentPicks = str("arenaRecentPicks", "Poslední výběry"),
                arenaEnded = str("arenaEnded", "ARÉNA UKONČENA"),
                arenaWinsWord = str("arenaWinsWord", "vítězství"),
                arenaRank0 = str("arenaRank0", "Příště to vyjde!"),
                arenaRank1 = str("arenaRank1", "Dobrý začátek."),
                arenaRank2 = str("arenaRank2", "Solidní výkon!"),
                arenaRank3 = str("arenaRank3", "Výborně! Jsi silný protivník."),
                arenaRank4 = str("arenaRank4", "Legenda arény!"),
                arenaWinsCount = str("arenaWinsCount", "Vítězství: %d"),
                arenaNextBattle = str("arenaNextBattle", "DALŠÍ BITVA"),
                arenaEnd = str("arenaEnd", "UKONČIT ARÉNU"),
                arenaYouLost = str("arenaYouLost", "Prohrál jsi"),
                arenaVictory = str("arenaVictory", "Vítězství!"),
                arenaDraw = str("arenaDraw", "Remíza"),
                resultYourCastleDestroyed = str("resultYourCastleDestroyed", "Tvůj hrad byl zničen."),
                resultEnemyCastleBuilt = str("resultEnemyCastleBuilt", "Nepřítel dokončil svůj hrad."),
                resultDrawEqual = str("resultDrawEqual", "Balíčky došly – hrady jsou stejně vysoké."),
                campaignPickHint = str("campaignPickHint", "Vyber lokaci a poraž\nvšechny soupeře"),
                campaignCleared = str("campaignCleared", "Vyčištěno"),
                campaignLocked = str("campaignLocked", "Zamčeno"),
                campaignDefeated = str("campaignDefeated", "Poražen"),
                campaignOppLocked = str("campaignOppLocked", "Zamčen"),
                campaignFight = str("campaignFight", "Bojuj!"),
                profileNamePrompt = str("profileNamePrompt", "Zadej své jméno hrdiny"),
                profileNameHint = str("profileNameHint", "Jméno hrdiny…"),
                profileEnterGame = str("profileEnterGame", "VSTOUPIT DO HRY"),
                back2 = str("back2", "Zpět"),
                rewardOnlineWin = str("rewardOnlineWin", "🌐 Online výhra"),
                rewardWin = str("rewardWin", "⚔️ Výhra"),
                rewardLoss = str("rewardLoss", "💀 Prohra"),
                dbResourceLabel = str("dbResourceLabel", "Zdroj:"),
                dbCraft = str("dbCraft", "Vyrobit  ✨%d"),
                dbManaCurve = str("dbManaCurve", "Mana křivka"),
                rogueBuildDeck = str("rogueBuildDeck", "SESTAV BALÍČEK"),
                rogueRarityLabel = str("rogueRarityLabel", "Rarita:"),
                rogueCostLabel = str("rogueCostLabel", "Cena:"),
                rogueDeckTitle = str("rogueDeckTitle", "ROGUELIKE — BALÍČEK"),
                rogueSavedDecks = str("rogueSavedDecks", "Uložené balíčky:"),
                rogueCardsCount = str("rogueCardsCount", "Karty: %d / %d"),
                rogueBudget = str("rogueBudget", "Rozpočet: %d / %d bodů"),
                rogueDeckEmpty = str("rogueDeckEmpty", "Balíček je zatím prázdný."),
                rogueFillDeck = str("rogueFillDeck", "Doplň balíček na %d karet."),
                rogueStartRun = str("rogueStartRun", "ZAHÁJIT RUN"),
                rogueDeckNotReady = str("rogueDeckNotReady", "BALÍČEK NENÍ HOTOVÝ"),
                rogueVictory = str("rogueVictory", "VÍTĚZSTVÍ"),
                rogueNext = str("rogueNext", "%s  ·  následuje %s"),
                roguePickRequired = str("roguePickRequired", "Vyber kartu (POVINNÉ) — zbývá %d×"),
                rogueAdd = str("rogueAdd", "PŘIDAT"),
                roguePickExtra = str("roguePickExtra", "Vyber jeden navíc, nebo přeskoč"),
                rogueNewMine = str("rogueNewMine", "Nový důl"),
                rogueSkip = str("rogueSkip", "Přeskočit"),
                rogueSurrenderMsg = str("rogueSurrenderMsg", "Rozehraný run bude ztracen. Opravdu se chceš vzdát?"),
                rogueYourDeck = str("rogueYourDeck", "TVŮJ BALÍČEK (%d)"),
                rogueRunComplete = str("rogueRunComplete", "RUN DOKONČEN!"),
                rogueRunOver = str("rogueRunOver", "RUN SKONČIL"),
                rogueAllBattles = str("rogueAllBattles", "Probil ses všemi %d bitvami."),
                rogueCastleFell = str("rogueCastleFell", "Tvůj hrad padl."),
                rogueBattlesWon = str("rogueBattlesWon", "Vyhrané bitvy: %d / %d"),
                rogueBattleLabel = str("rogueBattleLabel", "Bitva %d / %d"),
                rogueActTitles = str("rogueActTitles", "Hranice|Válečná pole|Citadela"),
                rogueEnemiesAct1 = str("rogueEnemiesAct1", "Goblin zvěd|Pěšák|Nájezdník|Lučištník|Zloděj"),
                rogueEnemiesAct2 = str("rogueEnemiesAct2", "Válečník|Sabotér|Temný učeň|Obléhatel|Žoldnéř"),
                rogueEnemiesAct3 = str("rogueEnemiesAct3", "Generál|Arcimág|Pán citadely|Válečný vládce|Katan"),
                replayWon = str("replayWon", "vyhrál"),
                replayLost = str("replayLost", "prohrál"),
                deckStarter = str("deckStarter", "Začátečník"),
                presetAttacker = str("presetAttacker", "⚔️ Útočník"),
                presetMage = str("presetMage", "🔮 Mágik"),
                presetDefender = str("presetDefender", "🏰 Obránce"),
                presetDefender2 = str("presetDefender2", "🏰 Obránce2"),
                presetCardsmith = str("presetCardsmith", "📚 Kartář"),
                presetSaboteur = str("presetSaboteur", "🕵️ Sabotér"),
            )
        }
